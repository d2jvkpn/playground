...
impl Matrix {
    ...
    pub fn random(rows: usize, cols: usize) -> Matrix {
        //assert!(rows * cols > 0, "Invalid rows or cols parameter");
    ...
    pub fn new(rows: usize, cols: usize, data: Vec<f64>) -> Matrix {
        ...
        assert!(rows * cols > 0 && rows * cols == data.len(), "Invalid Size");
        Matrix { rows, cols, data }
    }

    pub fn zeros(rows: usize, cols: usize) -> Matrix {
        //assert!(rows * cols > 0, "Invalid Size");
    ...
    pub fn dot_multiply(&self, other: &Matrix) -> Matrix {
        ...
        Matrix { rows: self.rows, cols: other.cols, data: result_data }
    }
    ...
    pub fn transpose(&self) -> Matrix {
        ...
        Matrix { rows: self.cols, cols: self.rows, data: buffer }
    }

    pub fn map(&mut self, func: fn(&f64) -> f64) -> Matrix {
        let mut result =
            Matrix { rows: self.rows, cols: self.cols, data: Vec::with_capacity(self.data.len()) };
