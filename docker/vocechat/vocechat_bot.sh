...
curl -i -k -X POST -H "User-Agent: $ua" -H "Origin: $server" -H "x-api-key: $api_key" \
  "$server/api/bot/send_to_user/$send_to_user" \
...
echo ">>> sending to group: $message"

curl -i -k -X POST -H "User-Agent: $ua" -H "Origin: $server" -H "x-api-key: $api_key"
  "$server/api/bot/send_to_group/$send_to_group"
  -H 'Content-Type: text/plain' --data-raw "$message"
