...
#### change password
docker exec -i mongo-mongos-1 mongosh --quiet /apps/bin/change-password.js
docker exec -it mongo-configsvr-1a mongosh  mongodb://127.0.0.1:27017/admin --quiet

# db.auth("root", "TheNewPassword");

docker exec -i mongo-shard-1a mongosh --quiet /apps/bin/change-password.js
docker exec -i mongo-shard-2a mongosh --quiet /apps/bin/change-password.js
docker exec -i mongo-shard-3a mongosh --quiet /apps/bin/change-password.js
