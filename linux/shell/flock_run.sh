...
#### 2. 🔐 Function: Try to acquire a file lock or exit if already locked
...
#### 3. 🧠 Main script logic starts here
echo "==> $(date +%FT%T%:z) Task started: command=$cmd, PID=$$"
"$@"
echo "<== $(date +%FT%T%:z)  Task completed: command=$cmd"

# ✅ No need to manually release the lock — it will be released when the script end
