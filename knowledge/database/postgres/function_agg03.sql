...
-- 步骤 2: 创建状态转换函数
CREATE OR REPLACE FUNCTION custom_jsonb_agg_status(state JSONB, value JSONB) RETURNS JSONB AS $$
BEGIN
...
-- 步骤 3: 创建最终函数
CREATE OR REPLACE FUNCTION custom_jsonb_agg_finnal(state JSONB) RETURNS JSONB AS $$
BEGIN
    RETURN state;
END;
$$ LANGUAGE plpgsql;
...
CREATE AGGREGATE custom_jsonb_agg(jsonb_data JSONB) (
    SFUNC = custom_jsonb_agg_status,
    STYPE = JSONB,
    FINALFUNC = custom_jsonb_agg_finnal,
    INITCOND = '{}'
);
